...
import com.algaworks.algashop.ordering.core.domain.model.customer.exception.CannotAddLoyaltyPointsToANonReadyOrder;
...
import com.algaworks.algashop.ordering.core.domain.model.order.exceptions.OrderNotBelongsToCustomerException;
...
class CustomerLoyaltyPointsApplicationServiceIT extends AbstractApplicationIT {
    ...
    @Test
    void shouldThrowCantAddLoyaltyPointsWhenOrderIsNotReady() {
        Customer customer = CustomerTestDataBuilder.brandNewCustomer().build();
        customers.add(customer);
        ...
        Assertions.assertThatExceptionOfType(CannotAddLoyaltyPointsToANonReadyOrder.class)
            .isThrownBy(() -> loyaltyPointsService.addLoyaltyPoints(customer.id().value(), order.id().toString()));
    }
