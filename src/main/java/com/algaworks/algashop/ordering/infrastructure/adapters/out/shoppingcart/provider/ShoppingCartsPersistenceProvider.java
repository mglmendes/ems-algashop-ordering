package com.algaworks.algashop.ordering.infrastructure.adapters.out.shoppingcart.provider;
...
public class ShoppingCartsPersistenceProvider implements ShoppingCarts {
    ...
    public void add(ShoppingCart aggregateRoot) {
        ...
        Optional<ShoppingCartPersistenceEntity> possibleEntity = persistenceRepository.findById(ShoppingCartId);

        if (possibleEntity.isPresent()) {
            update(aggregateRoot, possibleEntity.get());
        } else {
